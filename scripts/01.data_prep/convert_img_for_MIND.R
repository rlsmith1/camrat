...
# data --------------------------------------------------------------------
  ...
  base_dir <- "~/Documents/work/PhD/projects/CamRat/camrat/"

# ATLAS LABELS AND HIERARCHY
  load(paste0(base_dir, "objects/21June2023_sigma_labels.RDS")) # df_sigma_labels
...
# SCAN DIRECTORIES    
    ...
    mset_dir <- paste0(base_dir, "data/SIGMA_analysis/MT_scans/", study, "/", ses, "/mset/")
    infile_dir <- paste0(base_dir, "data/SIGMA_analysis/MT_scans/", study, "/", ses, "/infile/")
...
# READ ATLAS FOR IMAGE, COMBINE WITH MT VALUES FOR IMAGE, EXPORT
    for (sub in subjects) {
        ...
        # export as csv
        write.csv(df_mind,
                  file = paste0(base_dir, "data/SIGMA_analysis/MIND_files/", study, "/ROI/", ses, "_MT/input/", sub, ".csv"),
                  row.names = FALSE)
