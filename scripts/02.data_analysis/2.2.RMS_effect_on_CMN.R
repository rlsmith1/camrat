...
###
### Baseline degree and degree early developmental slope case-control differences & their relation to behavior
###
...
df_degree_gsk <- df_degree %>% filter(study == "GSK")

# BASELINE
df_degree_pnd20 <- df_degree_gsk %>%
...
# RELATE CASE-CONTROL DIFFERENCE IN BASELINE DEGREE TO MRC BASELINE DEGREE
df_baseline_comparison <- df_mt_degree_slopes %>% 
  unnest(cols = c(data)) %>% 
  ungroup %>% 
  filter(phenotype == "degree" & timepoint == 20 & study == "MRC" & region_of_interest %in% gray_matter_rois) %>% 
...
lm(t_value ~ feature_resids, data = df_baseline_comparison) %>% summary
lm(t_value ~ feature_resids + feature_resids2, data = df_baseline_comparison) %>% summary
lm(t_value ~ feature_resids + feature_resids2 + feature_resids3, data = df_baseline_comparison) %>% summary

# RELATE CASE-CONTROL DIFFERENCE IN DEGREE SLOPES TO MRC DEGREE SLOPES
df_slopes_comparison <- df_mt_degree_slopes %>% 
  unnest(cols = c(data)) %>% 
  ungroup %>% 
  filter(phenotype == "degree" & period == "early" & study == "MRC" & region_of_interest %in% gray_matter_rois) %>% 
...
lm(t_value ~ slope, data = df_slopes_comparison) %>% summary
lm(t_value ~ slope + slope2, data = df_slopes_comparison) %>% summary
lm(t_value ~ slope + slope2 + slope3, data = df_slopes_comparison) %>% summary

# EDGE
df_edge_gsk <-  df_edge %>% filter(study == "GSK")
df_edge_pnd20 <- df_edge_gsk %>%
  filter(timepoint == 20) %>%
  group_by(sys_edge) %>%
  nest() %>%
...
df_metric_behavior_cor_SYS <- df_degree_gsk %>% 
  ...
  group_by(timepoint, system, group, subject) %>% 
...
df_metric_behavior_cor_ROI <- df_degree_gsk %>% 
  filter(timepoint == 20) %>% # focus on baseline associations
...
# A: case-control baseline degree --------------------------------------------

f_plot_brain(df = df_degree_pnd20,
             contrast = "groupMS",
             labels = FALSE,
...
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/7a.case_control_baseline", .x), width = 7, height = 5)
)



# B: case-control slope ---------------------------------------------------

f_plot_brain(df = df_degree_lme,
  ...
  labs(title = "RMS-control differences in early development degree slope") +
  theme(legend.position = "bottom")

# SAVE
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/7b.case_control_degree_slope", .x), width = 7, height = 5)
)
...
df_centroids %>% 
  ...
  left_join(df_degree_gsk %>%
              ...
              mutate(feature = "Baseline degree"), 
            by = join_by(region_of_interest)
  ) %>% 
  bind_rows(
    df_centroids %>% 
...
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/7a2b2.degree_effect_size_coordinates", .x), width = 12, height = 3)
)


# C: Case-control baseline MT vs MRC baseline MT --------------------------

model <- lm(t_value ~ feature_resids, data = df_baseline_comparison) # model loses its significance if top two t-value ROIs are removed

df_baseline_comparison %>% 
  mutate(sign = ifelse(t_value < 0, "negative", "positive"),
         formula = f_model_label(model, type = "linear"),
  ...
  # geom_text(
  #   x = 2.5, y = 35, color = "blue", size = 2.5, hjust = 0.5,
  ...
  scale_shape_manual(values = c(21, 23)) +
  scale_size_manual(values = c(1, 3)) +
  labs(y = "Baseline degree in developmental cohort \n(corrected for TBV)",
       x = "RMS-control t-value at baseline",
       title = "Relationship between group effect size and baseline degree (at ROI level)") +
  coord_flip()

# SAVE
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/7c.case_control_baseline_MRC_baseline", .x), width = 5, height = 4)
)

# D: Case-control baseline MT vs MRC MT slope --------------------------

model <- lm(t_value ~ slope + slope2, data = df_slopes_comparison) # model loses its significance if lowest t-value ROIs is removed

df_slopes_comparison %>% 
  mutate(sign = ifelse(t_value < 0, "negative", "positive"),
         formula = f_model_label(model, type = "linear"),
         hubs = ifelse(region_of_interest %in% hubs, 1, 0) %>% factor(),
         label = ifelse(hubs == 1, str_replace_all(region_of_interest, "_", " "), NA)
  ...
  labs(y = "Degree slope in developmental cohort \n(corrected for TBV)",
       x = "RMS-control degree slope t-value at baseline",
       title = "Relationship between group effect size and degree slope (at ROI level)") +
  coord_flip()

# SAVE
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/7d.case_control_degree_slope_vs_MRC", .x), width = 5, height = 4)
)
...
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/7e.baseline_edge_case_control", .x), width = 8, height = 4) 
)



# S6a: Baseline degree boxplot -----------------------------------------------------------------------

f_plot_box(df = df_degree_pnd20,
           xlab = "", 
           ylab = "MT \n(corrected for sex + TBV)", 
           title = "Baseline regional degree profiles", 
           legend = FALSE) +
  coord_flip()
...
map(
  .x = c(".pdf", ".png"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/S6a.baseline_degree_boxplot", .x), width = 4, height = 3)
)

# S6b: ROI-specific degree slope differences ----------------------------------

f_plot_traj(df = df_degree_lme,
            timepoints = c(20, 63),
            xlab = "age (PND)", 
            ylab = "Degree (corrected for sex + TBV)", 
            title = "Differential degree trajectories in early development", 
            dim = c(2, 3),
            lnwdth = 0.75,
            legend = FALSE,
            include_individuals = TRUE
...
map(
  .x = c(".png", ".pdf"),
  .f = ~ ggsave(paste0(base_dir, "outputs/figures/S6b.ROI_degree_slope", .x), width = 8, height = 6)
)
